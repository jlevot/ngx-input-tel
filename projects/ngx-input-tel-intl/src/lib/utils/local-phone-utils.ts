...
export class LocalPhoneUtils {
     ...
     * @param searchField
     */
    public static isCountryMatching(country: Country, query: string, searchField: SearchCountryField): boolean {
        switch ( searchField ) {
            case 'DIALCODE':
                return country.dialCode.toLowerCase().includes(query.toLowerCase());
            case 'ISO2':
                return country.iso2.toLowerCase().includes(query.toLowerCase())
            case 'NAME':
                return country.name.toLowerCase().includes(query.toLowerCase())
            case 'ALL':
                return country.name.toLowerCase().includes(query.toLowerCase())
                    || country.iso2.toLowerCase().includes(query.toLowerCase())
                    || country.dialCode.toLowerCase().includes(query.toLowerCase())
        }
    }
