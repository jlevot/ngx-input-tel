...
export class ChangeData {
  ...
  constructor(phoneNumber?: PhoneNumber) {
    ...
    this.countryCode = phoneNumber ? utils.getRegionCodeForNumber(phoneNumber) : '';
    this.dialCode = phoneNumber ? `+${ phoneNumber.getCountryCode() }` : '';
    this.e164Number = phoneNumber ? utils.format(phoneNumber, PhoneNumberFormat.E164) : '';
